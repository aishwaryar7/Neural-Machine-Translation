...
def token2id(data, mode):
    ...
    in_file = open(os.path.join( PROCESSED_PATH, in_path), 'r',  encoding = 'utf8')
    out_file = open(os.path.join( PROCESSED_PATH, out_path), 'w',  encoding = 'utf8')
...
def load_data(enc_filename, dec_filename, max_training_size=None):
    encode_file = open(os.path.join(PROCESSED_PATH, enc_filename), 'r',  encoding = 'utf8')
    decode_file = open(os.path.join(PROCESSED_PATH, dec_filename), 'r',  encoding = 'utf8')
...
def _load_test_data():
    encode = load_test_data('tst2012.enc')
    decode = load_test_data('tst2012.dec')
...
def _eval_test_set(sess, model, test_buckets):
    ...
    for bucket_id in range(len(BUCKETS)):
        if len(test_buckets[bucket_id]) == 0:
            ...
            continue
        start = time.time()
        encoder_inputs, decoder_inputs, decoder_masks = get_batch(test_buckets[bucket_id], bucket_id, batch_size=BATCH_SIZE)
...
def _find_right_bucket(length):
    ...
    #print('_find_right_bucket', length)
...
def train():
    src_dictionary = dict()
    with open('data/vocab.enc', encoding='utf-8') as f:
        for line in f:
            src_dictionary[line[:-1]] = len(src_dictionary)

    print('ENC_VOCAB ', len(src_dictionary))

    tgt_dictionary = dict()
    with open('data/vocab.dec', encoding='utf-8') as f:
        for line in f:
            tgt_dictionary[line[:-1]] = len(tgt_dictionary)

    print('DEC_VOCAB', len(tgt_dictionary))
    ...
    with tf.Session() as sess:
        ...
        while iteration < MAX_ITERATION:
            ...
            if iteration % skip_step == 0:
                print('Loss {}: {}'.format(iteration, total_loss / skip_step))
                total_loss = 0
                saver.save(sess,'model/model', global_step=model.global_step)


def test():
    score = []

    _, enc_vocab = load_vocab('data/vocab.enc')
    inv_dec_vocab, _ = load_vocab('data/vocab.dec')
    with open('data/tst2013.dec', 'r', encoding='utf-8') as f:
        encode = f.readlines()

    with open('data/tst2013.enc', 'r', encoding='utf-8') as f:
        decode = f.readlines()
    ...
    with tf.Session() as sess:
        ...
        for i in range(len(encode)):
            token_ids = sentence2id(enc_vocab, str(encode[i]))
            ...
            score.append(nltk.translate.bleu_score.sentence_bleu([hypothesis.split()], reference, sf.method1))

    print('AVERAGE BLEU: ', np.mean(score))
...
def translate():
    ...
    with tf.Session() as sess:
        ...
        print('Welcome to NMT. Say something. Enter to exit. Max length is', max_length)
        while True:
            ...
            _, _, output_logits = run_step(sess, model, encoder_inputs, decoder_inputs,
                                           decoder_masks, bucket_id, True)
            response = _construct_response(output_logits, inv_dec_vocab)
            print('VIENAMESE Translation  ++++ ' + response + '\n')
